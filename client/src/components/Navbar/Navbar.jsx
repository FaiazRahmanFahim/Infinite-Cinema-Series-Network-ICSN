...
const Navbar = () => {
    ...
    return (
        <header className="sticky top-0 z-40 border-b border-base-300/60 bg-base-100/85 backdrop-blur-xl transition-all duration-300">
            <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
                    ...
                    <nav className="hidden items-center gap-1 lg:flex">
                        {navLinks.map(({ path, label, icon: Icon, color, isVip }) => (
                            ...
                            >
                                {({ isActive }) => (
                                    <>
                                        <Icon className={`h-4 w-4 ${isActive ? color : isVip ? 'text-amber-400' : 'opacity-70'}`} />
                                        <span>{label}</span>
                                        {isVip && (
                                            <span className="ml-0.5 rounded bg-gradient-to-r from-amber-500 to-orange-500 px-1 py-0.2 text-[9px] font-extrabold text-black uppercase tracking-wider">
                                                VIP
                                            </span>
                                        )}
                                    </>
                ...
                <div className="flex items-center gap-3">
                    {/* Interactive Search Bar (Desktop & large tablet) */}
                    <div className="relative hidden sm:block" ref={searchRef}>
                        <form onSubmit={handleSearchSubmit} className="relative">
                            ...
                            <input
                                ...
                                className="h-8 w-24 sm:w-28 md:w-32 lg:w-36 rounded-full border border-base-300/80 bg-base-200/50 pl-7 pr-6 text-xs font-medium text-base-content placeholder-base-content/40 transition-all duration-300 focus:w-44 focus:border-primary/60 focus:bg-base-100 focus:outline-none focus:ring-2 focus:ring-primary/20"
                            />
                            {searchValue && (
                                <button
                                    type="button"
                ...
                {sidebarOpen && (
                    <div className="fixed inset-0 z-50 lg:hidden">
                        ...
                        <motion.div
                            ...
                            className="fixed right-0 top-0 bottom-0 flex h-full w-80 sm:w-96 flex-col justify-between overflow-y-auto border-l border-base-300/80 bg-base-100/98 p-5 shadow-2xl backdrop-blur-2xl"
                            role="dialog"
